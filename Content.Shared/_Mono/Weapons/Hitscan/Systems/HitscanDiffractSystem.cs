...
public sealed partial class HitscanDiffractSystem : EntitySystem
{
        ...
        HitscanTraceEvent originalArgs, EntityCoordinates fromCoordinates, Vector2 direction)
    {
        ...
        var diffractedTraceEvent = new HitscanTraceEvent
        {
            ...
            Predicted = originalArgs.Predicted, // WOLFGATE(Weapons): the shooter's client draws the split beams too
        };

        RaiseLocalEvent(newHitscan, ref diffractedTraceEvent);
        QueueDel(newHitscan); // WOLFGATE(Weapons): the beam is done once traced; this was leaking an entity per split beam
    }
