...
public abstract partial class CESharedZLevelsSystem
{
    ...
    public readonly record struct CEWallContact(Box2 ShipTile, Box2 WallTile, EntityUid Wall); // WOLFGATE(PlanetCracker): retain the contacted obstacle for crash ploughing.
    ...
    private void WallCollides(Entity<MapGridComponent> grid, EntityUid wall, HashSet<EntityUid> walls, List<CEWallContact> contacts)
    {
        ...
        foreach (var tile in _map.GetTilesIntersecting(grid.Owner, grid.Comp, wallAabb))
        {
            ...
            if (shipAabb.Intersects(wallAabb))
                contacts.Add(new CEWallContact(shipAabb, wallAabb, wall)); // WOLFGATE(PlanetCracker)
        }
    }
