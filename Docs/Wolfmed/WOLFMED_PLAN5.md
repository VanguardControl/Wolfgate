...
- **ONYX** = `C:/tmp/onyx` @ `2f5bab9946539cbe083010c9ae6fbc59b47ae377`, sparse; absent paths read with
...
`git -C C:/tmp/onyx grep -n "circulatoryStream" HEAD -- Resources` returns exactly one hit, the declaration
...
- **`HurtCommand` part argument** — phase 6 (patch kept at `C:/tmp/wolfmed-plan/wp`).
