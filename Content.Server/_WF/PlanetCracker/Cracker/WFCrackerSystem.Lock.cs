...
public sealed partial class WFCrackerSystem
{
    ...
    /// <summary>A hull locked to its anchors cannot lift off.</summary>
    private void OnLiftoffAttempt(Entity<WFPlanetCrackerComponent> ent, ref WFLiftoffAttemptEvent args)
    {
        if (args.Cancelled || !ent.Comp.Locked)
            return;

        args.Cancelled = true;
        args.Reason = Loc.GetString("wf-liftoff-cracker-locked");
    }
}
