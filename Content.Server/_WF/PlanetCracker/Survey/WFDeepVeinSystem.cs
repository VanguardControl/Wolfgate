...
public sealed partial class WFDeepVeinSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<WFPlanetGroundSpawnedEvent>(OnGroundSpawned);
    }

    /// <summary>Adds the world's deep-vein marker layers after the planet's own, before any of the ground generates.</summary>
    private void OnGroundSpawned(ref WFPlanetGroundSpawnedEvent args)
    {
        if (!WFCrackSitePrototype.TryGet(_proto, args.Surface.ID, out var site)
            || !TryComp<BiomeComponent>(args.Ground, out var biome))
            return;

        foreach (var layer in site.MarkerLayers)
        {
            _biome.AddMarkerLayer(args.Ground, biome, layer);
        }
    }
    ...
    private bool TryGetVeinTable(EntityUid grid, out WFVeinTablePrototype table, out bool sanctioned)
    {
        ...
        return WFCrackSitePrototype.TryGet(_proto, surface.ID, out var site)
            && site.Veins is { } tableId
            && _proto.TryIndex(tableId, out table!);
    }
