...
public sealed partial class WFFissureSpawnerSystem : EntitySystem
{
    ...
    private bool TryGetFaction(EntityUid ground, out ProtoId<SalvageFactionPrototype>? faction, out bool sanctioned)
    {
        ...
        // A world with no crack site spawns no mobs.
        if (WFCrackSitePrototype.TryGet(_proto, surface.ID, out var site))
            faction = sanctioned ? site.Faction : site.UnsanctionedFaction ?? site.Faction;
