...
The owner can set a four-digit ship code and, on any Code or Chosen-people-or-code door, a code of the door's
own; both open such a door. Someone the door's rule does not admit right-clicks it and picks Enter Code, which
...
ship code as well, as the design table says ("its own, or the ship code"); a person without an account (an
NPC) can use a code but never locks out, since misses are per account; lockouts are round state and are not
