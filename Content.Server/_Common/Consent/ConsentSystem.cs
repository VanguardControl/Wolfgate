...
public sealed class ConsentSystem : SharedConsentSystem
{
    ...
    private void OnMindGotAdded(Entity<MindComponent> ent, ref MindGotAddedEvent args)
    {
        ...
        // WOLFGATE(Genitals): go through UpdateConsent so clients see the toggles and listeners get events on (re-)entry.
    ...
    {
        // WOLFGATE(Genitals) START: a visiting mind still owns its body, so that body's toggles stay current
        // A returnable ghost or shipyard preview would otherwise return to a body that still holds the old ones.
