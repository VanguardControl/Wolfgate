...
public sealed partial class TracerSystem : EntitySystem
{
    ...
    public void Draw(DrawingHandleWorld handle, MapId currentMap)
    {
        ...
        while (query.MoveNext(out var uid, out var tracer, out var xform)) // WOLFGATE
        {
            if (xform.MapID != currentMap)
                continue;

            // WOLFGATE START: a hidden projectile draws no trail, e.g. the server's copy of a shot the shooter predicted
