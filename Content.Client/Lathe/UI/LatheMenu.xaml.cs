...
public sealed partial class LatheMenu : FancyWindow
{
    ...
    private LatheRecipePrototype? _currentlyProducing;
    private bool _partsSiloLinked;
    ...
    private string GenerateTooltipText(LatheRecipePrototype prototype)
    {
        ...
        foreach (var (id, amount) in prototype.Entities)
        {
            ...
            if (missingAmount > 0 && !_partsSiloLinked)
        ...
        foreach (var (id, amount) in prototype.Reagents)
        {
            ...
            if (missingAmount > 0 && !_chemicalSiloLinked)
    ...
    public void PopulateQueueList(List<LatheRecipeBatch> queue) // Frontier: LatheRecipePrototype<LatheRecipeBatch
    {
        ...
        QueueEmptyLabel.Visible = queue.Count == 0;

        var idx = 1;
        var currentBatchMarked = false;
        foreach (var batch in queue) // Frontier: recipe<batch
        {
            var isPrinting = _currentlyProducing != null && !currentBatchMarked && _currentlyProducing.ID == batch.Recipe.ID;
            if (isPrinting)
                currentBatchMarked = true;

            var ready = isPrinting || idx - 1 < _queueReady.Count && _queueReady[idx - 1];

            var queuedRecipeBox = new PanelContainer();
            queuedRecipeBox.PanelOverride = new StyleBoxFlat
            {
                BackgroundColor = new Color(25, 31, 37),
                BorderColor = isPrinting ? new Color(57, 126, 114) : new Color(53, 66, 75),
    ...
    public void SetSupplyLinks(bool parts, bool chemicals)
    {
        _partsSiloLinked = parts;
        _chemicalSiloLinked = chemicals;
        PartsSiloStatus.Text = Loc.GetString(parts
            ? "lathe-menu-parts-silo-online"
            : "lathe-menu-parts-silo-offline");
        ChemicalSiloStatus.Text = Loc.GetString(chemicals
            ? "lathe-menu-chemical-silo-online"
            : "lathe-menu-chemical-silo-offline");
    }
    // </Mono>
