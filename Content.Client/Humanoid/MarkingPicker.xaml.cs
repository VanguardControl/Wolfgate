...
using Robust.Client.Graphics; // WOLFGATE(Humanoid)
...
public sealed partial class MarkingPicker : Control
{
    ...
    // WOLFGATE(Humanoid) START: list icon for a marking, not just Frame0
    /// <summary>
    ...
    /// </summary>
    private static Texture ListIcon(MarkingPrototype marking)
    {
        foreach (var sprite in marking.Sprites)
        {
            var one = new[] { sprite };
            var direction = WolfgateMarkingIcon.VisibleDirection(one, Direction.South);
            if (WolfgateMarkingIcon.AnyVisible(one, direction))
                return WolfgateMarkingIcon.FrameFor(sprite, direction);
        }
    ...
    private List<string> GetMarkingStateNames(MarkingPrototype marking)
    {
        ...
        foreach (var markingState in marking.Sprites)
        {
            switch (markingState)
            {
                // WOLFGATE(Genitals) START: split tail state names fall back through shared lookup instead of raw state
    ...
    public void Populate(string filter)
    {
        ...
        foreach (var marking in sortedMarkings)
        {
            ...
            var item = CMarkingsUnused.AddItem($"{GetMarkingName(marking)}", ListIcon(marking)); // WOLFGATE(Humanoid): icon from a facing the marking is visible in
    ...
    {
        CMarkingsUsed.Clear();
        CMarkingColorsCard.Visible = CMarkingColors.Visible = false; // WOLFGATE(Humanoid)
        ...
        foreach (var marking in _currentMarkings.GetReverseEnumerator(_selectedMarkingCategory))
        {
            ...
            {
                Text = text,
                Icon = ListIcon(newMarking), // WOLFGATE(Humanoid): icon from a facing the marking is visible in
                Selectable = true,
                Metadata = newMarking,
    ...
    private void OnUsedMarkingSelected(ItemList.ItemListSelectedEventArgs item)
    {
        ...
        List<WolfgateColorPicker> colorSliders = new(); // WOLFGATE(UserInterface): swatches + sliders
        for (int i = 0; i < prototype.Sprites.Count; i++)
        {
            ...
            // WOLFGATE(Genitals) START: ported from HardLight/Floof, a colour-linked sprite gets no picker of its own
            // The selector is still created and kept in the list so the remaining pickers stay aligned with their
            // colour indices.
            ...
            WolfgateColorPicker colorSelector = new WolfgateColorPicker(); // WOLFGATE(UserInterface)
        ...
        CMarkingColorsCard.Visible = CMarkingColors.Visible = true; // WOLFGATE(Humanoid)
    }
    ...
    private void MarkingAdd()
    {
        ...
        var item = new ItemList.Item(CMarkingsUsed)
        {
            ...
            Icon = ListIcon(marking), // WOLFGATE(Humanoid): icon from a facing the marking is visible in
            Selectable = true,
            Metadata = marking,
        };
